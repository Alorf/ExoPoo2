...
public class Gestion {
    ...
    private void menu() {
        List<String> options = new ArrayList<>(Arrays.asList("auteurs", "ouvrages", "exemplaires", "rayons", "lecteurs", "locations", "fin"));
    ...
    private void gestLocations() {
        ...
        Lecteur loueur;
        do {
            System.out.println("Choisissez un exemplaire");
            ...
            if (choix < 1 || choix > lex.size()) {
                System.out.println("Erreur, recommencez");
            } else {
                exemplaire = lex.get(choix - 1);
                break;
            }
        } while (true);
        ...
        do {
            ...
            if (choix < 1 || choix > llect.size()) {
                System.out.println("Erreur, recommencez");
            } else {
                loueur = llect.get(choix - 1);
                break;
            }
        } while (true);

        lloc.add(new Location(loueur, exemplaire));
    }
        ...
        switch (choix) {
            case 1:
                ...
                do {
                    ...
                    for (TypeLivre tlist : TypeLivre.values()) {
                        System.out.println(tlist);
                    }
                    tll = TypeLivre.valueOf(sc.nextLine().toUpperCase());
                    ...
                    if (!flag){
                        System.out.println("Erreur, recommencez votre saisie");
                    }
                } while (!flag);
            ...
            case 2:
                ...
                System.out.printf("Nombre de plage : ");
                byte nbrePlages = sc.nextByte();
                sc.skip("\n");
                System.out.printf("Durée totale : ");
                String dureetotale = sc.nextLine();


                o = new CD(titre, ageMin, dp, ploc, langue, genre, codeCD, nbrePlages, dureetotale);
                ;
                break;
            case 3:
                System.out.print("Code : ");
                long code = sc.nextLong();
                //Skip quand int puis String
                sc.skip("\n");
                System.out.printf("Durée totale : ");
                String dt = sc.nextLine();
                System.out.printf("Nombre bonus : ");
                byte nbonus = sc.nextByte();
                o = new DVD(titre, ageMin, dp, ploc, langue, genre, code, dt, nbonus);
                ;
                break;
        }
        louv.add(o);
